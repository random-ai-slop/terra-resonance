...
def GradVandermonde1D(N,r):
    ...
    #print (np.vstack((DVr, GradJacobiP(r,0,0,1))))
    for i in range(1,N+1):
        DVr = np.vstack((DVr, GradJacobiP(r,0,0,i)))# check how to merge them together
...
def JacobiGQ(alpha,beta,N):
    ...
    x,V = np.linalg.eigh(J) #the eigenvalues are from large to small, different from matlab
    #print (V,J)
    #w = np.power((V[0,:].T),2)*(2**(alpha+beta+1))/(alpha+beta+1)*gamma(alpha+1)*gamma(beta+1)/gamma(alpha+beta+1)
    return np.sort(x)#,w # x and w are in different oder as in matlab
...
def Vandermonde1D(N,r):
    ...
    for j in range(1,N+1):
        V1D = np.vstack((V1D, JacobiP(r,0,0,j))) # JacobiP(r,0,0,j) is reduced to one dimention.
