...
from earth_modes.vendor.Ouroboros.constants import G
from earth_modes.vendor.Ouroboros.modes import lib
...
def _fluid_density_interfaces(A11, model, order, rho, radius):
    """Assemble distributional -rho' g r² U test(U) inside a fluid domain.

    Material jumps can lie inside one continuous fluid phase domain. The volume
    rho_p array contains ordinary within-layer derivatives only. At each repeated
    material radius, rho' additionally contains [rho] delta(r-rb). Domain ends
    are excluded: their traction terms are assembled by compute_modes.
    Solid weak forms already eliminate rho' by integration by parts and must
    not receive this extra term.
    """
    radius = np.asarray(radius)
    rho = np.asarray(rho)
    for j in np.flatnonzero(np.diff(radius) == 0):
        boundary = radius[j]
        if not model.x[0, 0] < boundary < model.x[-1, -1]:
            continue
        delta = rho[j + 1] - rho[j]
        if delta == 0:
            continue
        edges = np.flatnonzero(np.isclose(model.x[-1], boundary, rtol=0,
                                         atol=64*np.finfo(float).eps*max(radius)))
        if len(edges) != 1:
            raise ValueError('Fluid material interface must coincide with one element edge')
        index = (int(edges[0]) + 1) * order
        gravity = float(lib.gravfield_lst(np.array([boundary]), rho, radius)[0])
        A11[index, index] -= delta * gravity * boundary**2
...
def radial_fluid_GP_mixedPV(model,invV,invV_p,invV_P,order,order_p,order_P,Dr,Dr_p,Dr_P,rho,radius):
    ...
    _fluid_density_interfaces(A11, model, order, rho, radius)
