...
def gravfield(r,rho,radius):
    """Newton gravity, integrating the model's linear density on each side."""
    if r <= 0:
        return 0.0
    mass = 0.0
    for i in range(len(radius)-1):
        lo, hi = radius[i], radius[i+1]
        if lo >= r: break
        if hi <= lo: continue
        top = min(r,hi)
        slope = (rho[i+1]-rho[i])/(hi-lo)
        intercept = rho[i]-slope*lo
        mass += intercept*(top**3-lo**3)/3+slope*(top**4-lo**4)/4
    return 4*np.pi*G*mass/(r*r)
